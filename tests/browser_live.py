...
if args.only:
    ...
    if not selected: parser.error('Unknown case name')
results = []
with sync_playwright() as p:
    ...
    for case in selected:
        ...
        for question in case['turns']:
            ...
            with page.expect_response(lambda response: response.url.endswith('/api/chat'), timeout=360000) as pending:
        ...
        missing = [facts for facts in case['facts'] if not any(fact_present(f, reply) for f in facts)]
        ...
        results.append({'name': case['name'], 'passed': passed, 'missing': missing, 'reply': reply})
        print(f"{'PASS' if passed else 'FAIL'} UI {case['name']} {missing if missing else ''}", flush=True)
...
assert all(r['passed'] for r in results), 'Some UI acceptance cases failed; see browser-live-results.json'
