...
with sync_playwright() as p:
    ...
    expect(
        page.get_by_role(
            "heading", name="Every deal has a story. Get the full picture."
        )
    ).to_be_visible()
    ...
    answers = [
        {
            ...
            "tool_calls": [
                {
                    "name": "resolve_entity",
                    "args": {"query": "Acme", "entity_type": "organization"},
            ...
            "tool_calls": [
                {
                    ...
                    "args": {"org_id": "org-acme-labs"},
                    "result": {"leads": [{"id": "lead-6", "amount": 48000}]},
                }
            ],
        },
    ]
    ...
    def chat(route):
        ...
        route.fulfill(
            status=200, content_type="application/json", body=json.dumps(answers.pop(0))
        )
    ...
    browser.close()
    print(
        "Browser smoke passed: real API setup error, mocked chat, clarification, session continuity, traces, reset, narrow viewport."
    )
