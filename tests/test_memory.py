...
def test_resolution_and_read_only_copies():
    ...
    assert (
        tools.resolve_entity("Acme Cloud", "organization")["entity"]["id"]
        == "org-acme-cloud"
    )
    assert tools.resolve_entity("zzzzzzzz", "lead")["status"] == "not_found"
    record = tools.get_lead_details("lead-1")
    record["amount"] = 1
...
def test_structured_rank_and_comparison():
    ...
    assert (last["name"], last["amount"], last["closed_date"]) == (
        "Notion - Sales Workspace",
        96000,
        "2026-09-02",
    )
    groups = tools.compare_deal_groups(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("rank_leads", {"limit": 0}),
        ("rank_leads", {"order_by": "bogus"}),
        ("rank_leads", {"stage": "bogus"}),
        (
            "compare_deal_groups",
            {
                "group_a_filter": {"stage": "closed_won", "typo": 1},
                "group_b_filter": {"stage": "closed_lost"},
            },
        ),
...
def test_tied_latest_requires_clarification(monkeypatch):
    ...
    assert (
        tools.rank_leads(stage="closed", order_by="closed_date", limit=1)["status"]
        == "ambiguous"
    )
...
def test_lazy_cache_and_cosine():
    class Encoder:
        ...
        def embed(self, texts):
            ...
            return [
                np.array([1, 0]) if "budget" in t else np.array([0, 1]) for t in texts
            ]
    ...
    chunks = [
        {"id": "a", "text": "budget frozen"},
        {"id": "b", "text": "security approval"},
    ]
