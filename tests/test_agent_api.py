...
def calls(*items):
    return types.Content(
        ...
        parts=[
            types.Part(
                function_call=types.FunctionCall(name=name, args=args, id=str(i))
            )
...
def test_ambiguity_stops_batch_then_resumes_ordinal():
    model = ScriptedModel(
        calls(
            ("resolve_entity", {"query": "Acme", "entity_type": "organization"}),
...
def test_api_chat_trace_validation_and_cors(monkeypatch):
    model = ScriptedModel(
        calls(("rank_leads", {"stage": "prospect"})),
        answer("Vercel $110,000; Figma $64,000."),
    )
    monkeypatch.setattr(main, "agent", Agent(model))
    client = TestClient(main.app)
    response = client.post(
        "/chat", json={"session_id": "test", "message": "Top prospects"}
    )
    assert response.status_code == 200
    assert response.json()["tool_calls"][0]["result"][0]["amount"] == 110000
    assert (
        client.post("/chat", json={"session_id": "test", "message": " "}).status_code
        == 422
    )
    assert (
        client.post("/chat", json={"session_id": "../", "message": "Hi"}).status_code
        == 422
    )
    cors = client.options(
        "/chat",
...
def test_missing_key_is_actionable(monkeypatch):
    ...
    response = TestClient(main.app).post(
        "/chat", json={"session_id": "test", "message": "Hi"}
    )
...
def test_unclear_followup_cannot_resume_tools():
    model = ScriptedModel(
        calls(("resolve_entity", {"query": "Alex", "entity_type": "person"}))
    )
    agent = Agent(model)
...
def test_provider_retries_transient_errors_and_sanitizes_failures(monkeypatch):
    ...
    results = iter(
        [
            ClientError(429, {"error": {"message": "secret-bearing upstream URL"}}),
