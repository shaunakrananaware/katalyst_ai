...
def safe(fn):
    ...
    def wrapped(*args, **kwargs):
        try:
            ...
            return {'status': 'unavailable', 'message': 'Retrieval failed. Try again; do not infer missing evidence.'}
    return wrapped

def node(nid, kind):
    if nid not in GRAPH or GRAPH.nodes[nid]['type'] != kind: raise KeyError(nid)
    return deepcopy(dict(GRAPH.nodes[nid]))

def related(nid, edge_type, incoming=False):
    edges = GRAPH.in_edges(nid, data=True) if incoming else GRAPH.out_edges(nid, data=True)
...
def bound(value, maximum=100):
    if value is not None and not 1 <= value <= maximum: raise ValueError(f'limit must be 1..{maximum}')
...
def capped(rows, limit, field):
    ...
    if limit and len(rows) > limit and rows[limit-1][field] == rows[limit][field]:
        ...
        return {'status': 'ambiguous', 'reason': f'Tied {field} at selection boundary', 'candidates': tied}
    return rows[:limit] if limit else rows

@safe
def resolve_entity(query: str, entity_type: Literal['lead', 'organization', 'person']) -> dict:
    ...
    scored = sorted([(WRatio(query, n['name'].casefold()), n) for n in rows], key=lambda x: (-x[0], x[1]['id']))
    candidates = exact or [n for score, n in scored if score >= 70 and score >= scored[0][0]-8]
    if not candidates: return {'status': 'not_found', 'query': query}
    if len(candidates) > 1: return {'status': 'ambiguous', 'candidates': deepcopy(candidates)}
...
def get_lead_details(lead_id: str) -> dict:
    ...
    return {**lead, 'organization': related(lead_id, 'BELONGS_TO')[0], 'owner': related(lead_id, 'OWNED_BY')[0]}

@safe
def get_meetings_for_lead(lead_id: str, limit: int | None = None, most_recent_first: bool = True) -> list[dict] | dict:
    """Retrieve meetings by date; tied latest selections return ambiguous candidates."""
    node(lead_id, 'lead'); bound(limit)
    rows = sorted(related(lead_id, 'FOR_LEAD', True), key=lambda n: n['datetime'], reverse=most_recent_first)
...
def get_emails_for_lead(lead_id: str, limit: int | None = None) -> list[dict] | dict:
    ...
    node(lead_id, 'lead'); bound(limit)
    return capped(sorted(related(lead_id, 'ABOUT_LEAD', True), key=lambda n: n['sent_at'], reverse=True), limit, 'sent_at')

@safe
def rank_leads(stage: str | None = None, order_by: Literal['amount', 'created_date', 'closed_date'] = 'amount', descending: bool = True, limit: int = 5) -> list[dict] | dict:
    ...
    bound(limit)
    if stage is not None and stage not in STAGES | {'closed'}: raise ValueError('Unknown stage')
    rows = [dict(n) for _, n in GRAPH.nodes(data=True) if n['type'] == 'lead'
            and (stage is None or n['stage'] == stage or (stage == 'closed' and n['closed_date']))
            and n.get(order_by) is not None]
    return capped(sorted(rows, key=lambda n: n[order_by], reverse=descending), limit, order_by)
...
def text_chunks(lead_id):
    ...
    for meeting in related(lead_id, 'FOR_LEAD', True):
        for note in related(meeting['id'], 'NOTE_OF', True):
            chunks.append({'id': note['id'], 'source': 'Notion', 'meeting_id': meeting['id'],
                           'date': meeting['datetime'], 'text': note['text']})
    for email in related(lead_id, 'ABOUT_LEAD', True):
        chunks.append({'id': email['id'], 'source': 'Gmail', 'date': email['sent_at'], 'text': email['body']})
    return chunks

@safe
def compare_deal_groups(group_a_filter: dict, group_b_filter: dict, limit_per_group: int = 5) -> dict:
    ...
    for label, criteria in [('group_a', group_a_filter), ('group_b', group_b_filter)]:
        if set(criteria) != {'stage'} or criteria['stage'] not in STAGES: raise ValueError('Each filter requires a valid stage only')
        rows = rank_leads(stage=criteria['stage'], order_by='closed_date' if criteria['stage'].startswith('closed') else 'created_date', limit=limit_per_group)
        if isinstance(rows, dict): return rows
        groups[label] = {'filter': criteria, 'requested_count': limit_per_group, 'count': len(rows),
                         'total_amount': sum(n['amount'] for n in rows), 'currency': 'USD',
                         'deals': [{**n, 'evidence': text_chunks(n['id'])} for n in rows]}
    return groups

@safe
def semantic_search_lead_text(lead_id: str, query: str, top_k: int = 3) -> list[dict] | dict:
...
def get_org_context(org_id: str) -> dict:
    ...
    for person in people:
        for meeting in related(person['id'], 'ATTENDED_BY', True):
            for lead in related(meeting['id'], 'FOR_LEAD'): leads[lead['id']] = lead
        for edge in ('SENT_BY', 'SENT_TO'):
            for email in related(person['id'], edge, True):
                for lead in related(email['id'], 'ABOUT_LEAD'): leads[lead['id']] = lead
        for lead in related(person['id'], 'OWNED_BY', True): leads[lead['id']] = lead
    return {'organization': org, 'people': people, 'leads': list(leads.values())}

TOOL_FUNCTIONS = [resolve_entity, get_lead_details, get_meetings_for_lead, get_notes_for_meeting,
                  get_emails_for_lead, rank_leads, compare_deal_groups, semantic_search_lead_text, get_org_context]
